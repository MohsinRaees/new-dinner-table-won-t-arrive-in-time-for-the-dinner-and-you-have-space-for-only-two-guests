...
while (dinnerGuests.length > 2) {
    let removedGuest: string | undefined = dinnerGuests.pop();
    if (removedGuest) {
...
dinnerGuests.forEach((remainingGuest: string) => {
    console.log(`Dear ${remainingGuest}, you're still invited to dinner. Please join us!`);
});
